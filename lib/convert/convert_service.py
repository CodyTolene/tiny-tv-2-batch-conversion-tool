...
class ConvertService:
    def __init__(
        ...
        *,
        ...
        out_h: int,
        fps: int,
        vcodec: str,   # kept for ctor compatibility, not used
        acodec: str,   # kept for ctor compatibility, not used
        arate: str,    # kept for ctor compatibility, not used
        ach: str,      # kept for ctor compatibility, not used
        on_log: LogFn,
    ...
    # Preview helpers (left as-is in case your UI uses them)
    ...
    def build_cmd(self, src: Path, dst: Path, q_val: int) -> list[str]:
        vf = (
            f"scale={self.out_w}:{self.out_h}:force_original_aspect_ratio=increase,"
            f"crop={self.out_w}:{self.out_h}:exact=1,hqdn3d"
        )
    ...
    def convert_files(
        ...
        *,
        output_dir: Path,
        q_val: int,
        scale_mode: str,       # unused now
        normalize_audio: bool, # unused now
        ...
        def worker():
            ...
            for idx, src in enumerate(files, start=1):
                ...
                cmd = self.build_cmd(src, dst, q_val)
